...
// Digits of `n` from its highest non-zero place down to the units (E).
function placesOf(n: number, maxGetal: number) {
    const all = getMaskPlaces(Math.max(maxGetal, n), 'natural');   // descending D H T E …
    const startIdx = all.findIndex(p => Math.floor(n / p.weight) % 10 !== 0);
    const slice = startIdx < 0 ? all.slice(-1) : all.slice(startIdx);
    return slice.map(p => ({ key: p.key, label: p.label, weight: p.weight, digit: Math.floor(n / p.weight) % 10 }));
}
...
export default function PlaatswaardeViewer({ block, showSolutions }: Props) {
    ...
    // Render the number with the targeted digit underlined.
    const numberWithUnderline = (ex: PlaatswaardeExercise) => {
        const places = placesOf(ex.number, maxGetal);
        return (
            <span style={{ fontFamily: mono, fontSize: '18px', letterSpacing: '1px' }}>
                {places.map(p => (
                    <span key={p.key} style={p.key === ex.placeKey ? { borderBottom: '2px solid #000', padding: '0 1px' } : undefined}>
                        {p.digit}
                    </span>
                ))}
            </span>
    ...
    return (
        <FragmentableGrid
            ...
            rowGap={gap}
            items={exercises.map(ex => {
                const place = placesOf(ex.number, maxGetal).find(p => p.key === ex.placeKey)!;
                const answer = subType === 'plaats' ? place.label.toLowerCase() : formatMathNumber(place.digit * place.weight);
                return (
